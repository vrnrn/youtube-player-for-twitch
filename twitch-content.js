...
(function () {
    ...
    const TWITCH_CHANNEL_PATTERN = /^\/([a-zA-Z0-9_]+)/;
    ...
    function getTwitchChannel() {
        ...
        const match = window.location.pathname.match(TWITCH_CHANNEL_PATTERN);
    ...
    async function searchYouTubeLive(channelName) {
        ...
        try {
            ...
            // Find best match
            const normalizedChannel = channelName.toLowerCase().replace(NON_ALPHANUMERIC_PATTERN, '');

            // 1. Exact/Close Match
            for (const video of contents) {
                const normalizedResult = video.channel.toLowerCase().replace(NON_ALPHANUMERIC_PATTERN, '');
    ...
    function setupGlobalListeners() {
        ...
        document.addEventListener('keydown', (e) => {
            ...
            if (e.altKey && e.key.toLowerCase() === 'y') {
                ...
                e.preventDefault();
                const btn = document.querySelector('[data-a-target="player-theater-mode-button"], [aria-label*="Theater Mode"]');
                if (btn) btn.click();
            }
        });
